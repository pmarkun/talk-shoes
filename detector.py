...
class ShoesAIAnalyzer:
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.classes = self._load_classify_shoes_classes(
            self.config["models"]["classify_shoes_classes"]
        )
        self.classify_shoes_model = self._load_classify_shoes_model(
            self.config["models"]["classify_shoes_model"]
        )

        # Carrega o Detector de Tênis
        self.detect_shoes_model = self._load_yolo_model(
            self.config["models"]["detect_shoes_model"]
        )

        # Carrega o Detector de Pessoas
        self.detect_person_model = self._load_yolo_model(
            self.config["models"]["detect_person_model"]
        )

        # Carrega o Detector de Placa de Peito (Bib)
        bib_model_path = self.config["models"].get("detect_bib_model")
        self.detect_bib_model = (
            self._load_yolo_model(bib_model_path) if bib_model_path else None
        )

        # Categorias e instruções de cor para corrida (opcionais)
        self.bib_settings = self.config.get("settings", {}).get("bib_detect", {})

        # Carrega o Detector de Faces
        self.detect_face_model = FaceAnalysis(
            name="buffalo_l",
            allowed_modules=["detection"],
            providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        )
        self.detect_face_model.prepare(ctx_id=0, det_size=(640, 640))
        self.detect_face_transform = (
            self._define_transform()
        )  # Pode ser igual ou diferente se necessário para FairFace

        # Carrega o classificador de gênero
        self.classify_gender_model = self._load_classify_gender_model(
            self.config["models"]["classify_gender_model"]
        )
        self.classify_transform = (
            self._define_transform()
        )  # Transformação padrão para classificação ViT e gênero

        self.confidence = self.config["settings"][
            "confidence"
        ]  # Confiança mínima para detecção de face
    ...
    def _load_classify_shoes_classes(self, classes_path) -> list:
        ...
        try:
            ...
            if not classes:
                logger.warning(
                    f"Arquivo de classes {classes_path} está vazio ou não contém classes válidas."
                )
                return []
    ...
    def _load_classify_gender_model(self, model_path: str) -> torch.nn.Module:
        ...
        try:
            model = models.resnet34(
                weights=None
            )  # Use weights=None para evitar warning de deprecated 'pretrained'
    ...
    def _load_classify_shoes_model(self, model_path: str) -> torch.nn.Module:
        ...
        try:
            ...
            logger.error(
                f"Arquivo do modelo de classificação {model_path} não encontrado."
            )
            raise
        except Exception as e:
    ...
    def _load_yolo_model(self, model_path: str):
        ...
        try:
            ...
            FileNotFoundError
        ):  # YOLO pode levantar outros erros se o arquivo estiver corrompido
    ...
    def detect_person_batch(
    ...
    ) -> list[tuple[Path, np.ndarray | None, list]]:
        """
        ...
        valid_pil_images = [
            data[1] for data in loaded_images_data if data[1] is not None
        ]

        if not valid_pil_images:
            return [
                (p, None, []) for p, _ in loaded_images_data
            ]  # Retorna Nones para todas

        np_imgs_for_yolo = [np.array(pil_img) for pil_img in valid_pil_images]

        verbose_level = (
            logger.getEffectiveLevel() <= logging.DEBUG
        )  # Seja verboso se INFO ou DEBUG
        yolo_batch_results = list(
            self.detect_person_model(
                np_imgs_for_yolo, iou=0.5, stream=True, verbose=verbose_level
            )
        )
        ...
        for original_path, original_pil_img in loaded_images_data:
            if original_pil_img:
                ...
                np_array = np_imgs_for_yolo[
                    yolo_result_idx
                ]  # Reusa o array já convertido
                yolo_res = yolo_batch_results[yolo_result_idx]
                output_results.append(
                    (original_path, np_array, [yolo_res])
                )  # YOLO retorna lista de resultados, mesmo para uma img
                yolo_result_idx += 1
            else:
    ...
    def classify_batch(self, pil_images: list[Image.Image]) -> list[dict]:
        ...
        with torch.no_grad():
            logits = self.classify_shoes_model(
                batch
            ).logits  # Para ViTForImageClassification
    ...
    def detect_faces(self, pil_image: Image.Image) -> list[Image.Image]:
        ...
        logger.debug(
            f"Retornando {len(face_crops)} rostos após filtro de confiança ({self.confidence})."
        )
        return face_crops
    ...
    def _pick_primary_face(
        ...
        if shoes:
            scx = sum(
                (sx1 + sx2) / 2 for s in shoes for sx1, _, sx2, _ in [s["bbox"]]
            ) / len(shoes)
        else:
            scx = pcx
    ...
    def classify_gender_batch(self, face_pil_images: list[Image.Image]) -> list[dict]:
        ...
        age_labels = [
            ...
            "60-69",
            "70+",
        ]  # Ajustado range para ser consistente
        race_labels = [
            "White",
            "Black",
            "Latino_Hispanic",
        ...
        for i in range(len(face_pil_images)):
            demographic_results.append(
                {
                    "gender": {
                        "label": gender_labels[int(gender_idx[i].item())],
                        "prob": max_gender_p[i].item(),
                    },
                    "age": {
                        "label": age_labels[int(age_idx[i].item())],
                        "prob": max_age_p[i].item(),
                    },
                    "race": {
                        "label": race_labels[int(race_idx[i].item())],
                        "prob": max_race_p[i].item(),
                    },
    ...
    def _create_initial_record(
        self, image_path: Path, folder_root: Path, width: int = 0, height: int = 0
    ) -> dict:
    ...
    def process_images(
        ...
        logger.info(
            f"Iniciando processamento de imagens em '{folder}', confidence={confidence}"
        )
        folder_path = Path(folder)
        valid_ext = {".jpg", ".jpeg", ".png", ".webp"}

        image_files = [
            p for p in folder_path.rglob("*") if p.suffix.lower() in valid_ext
        ]
        image_files.sort()
        ...
        for i in tqdm(
            range(0, len(image_files), batch_size), desc="Processando Lotes de Imagens"
        ):
            ...
            batch_image_paths = [
                ...
                if str(p.relative_to(folder_path)) not in processed
            ]
            ...
            for image_path, img_array, yolo_results_list in batch_detect_person_data:
                if cancel_event is not None and cancel_event.is_set():
                    logger.info(
                        "Cancelamento solicitado - interrompendo o processamento do lote atual."
                    )
                    break
                if img_array is None:  # Imagem falhou ao carregar
                    logger.warning(
                        f"Pulando processamento adicional para imagem com falha na leitura: {image_path}"
                    )
                    continue
                ...
                person_crops_data = self.crop_images(
                    img_array, yolo_results_list, confidence=0.9, classes_to_crop=[0]
                )  # Classe 0 para sapatos

                if person_crops_data:
                    for person_crop in person_crops_data:
                        record = self._create_initial_record(
                            image_path, folder_path, image_width, image_height
                        )
                        record["bbox"] = person_crop["box"]
                        face_crops_pil = self.detect_faces(
                            person_crop["img"]
                        )  # Usa o método que retorna PIL images
                        if face_crops_pil:
                            demographic_data_list = self.classify_gender_batch(
                                face_crops_pil
                            )
                        else:
                            demographic_data_list = []
                        shoe_crops = self.detect_shoes_model(
                            person_crop["img"], stream=True, verbose=False
                        )  # Executa detecção de sapatos
                        shoe_crops_data = self.crop_images(
                            np.array(person_crop["img"]),
                            shoe_crops,
                            classes_to_crop=[0],
                        )
                        shoe_classifications = self.classify_batch(
                            [shoe_crop["img"] for shoe_crop in shoe_crops_data]
                        )
                        shoes = []
                        for crop_info, classification_info in zip(
                            shoe_crops_data, shoe_classifications
                        ):
                            foot = {}
                            ...
                            self._save_shoe_crop(
                                crop_info["img"], foot["label"], foot["prob"]
                            )
                            shoes.append(foot)

                        bibs = None
                        if (
                            self.bib_settings.get("enabled", False)
                            and self.detect_bib_model
                        ):
                            try:
                                ...
                                sleep(1)  # Espera para evitar problemas de concorrência
                                bibs = self.detect_bibs(person_crop["img"])
                        record["shoes"] = shoes
            ...
            if batch_records:
                ...
                if output_path:
                    try:
                        ...
                        logger.error(
                            f"Erro ao salvar dados parciais em {output_path}: {e}"
                        )
            ...
            if progress_callback:
                progress_callback(len(df_full), len(image_files))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Análise de marcas de calçados e demografia em imagens."
    )
    parser.add_argument("folder", help="Pasta contendo as imagens a serem analisadas.")
    parser.add_argument(
        "--output",
        default="output/processed_dataset.json",
        help="Pasta para salvar os resultados (dataset.json, tasks).",
    )
    parser.add_argument(
        "--bib-categories",
    ...
    if args.folder:
        ...
        if args.resume and Path(args.output).is_file():
            try:
                ...
                logger.info(
                    f"Retomando processamento a partir de {len(skip_files)} registros."
                )
            except Exception as e:
        ...
        try:
            ...
            logger.error(
                f"Erro ao salvar DataFrame processado em {df_output_path}: {e}"
            )
